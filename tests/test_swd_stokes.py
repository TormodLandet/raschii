...
def test_swd_stokes_amp2(depth, tmpdir):
    ...
    WaveModel, _ = raschii.get_wave_model("Stokes")
    wave = WaveModel(height=height, depth=depth, length=length, N=norder)
    ...
    eff_depth = 50 * math.pi / wave.k if depth < 0 else min(depth, 50 * math.pi / wave.k)
...
@pytest.mark.parametrize("depth", [15.0, -1.0])
def test_swd_stokes_amp3(depth, tmpdir):
    """amp=3: elevation-only file — c data absent, elevation correct."""
    from raschii.swd import SwdReaderForRaschiiTests

    file_swd = tmpdir / "stokes_amp3.swd"
    height = 5.0
    length = 200.0
    nperiods = 0.4
    dt = 0.1
